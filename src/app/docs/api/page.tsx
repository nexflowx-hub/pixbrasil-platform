  ...
  return (
    ...
    >
      ...
      <DocsSection id="create" title="POST /payments/charge" description="Cria um PaymentIntent idempotente e resolve Store → policy → provider → economics → release.">
        ...
        <div className="mt-4 rounded-2xl border border-[#20F29A]/16 bg-[#20F29A]/4 p-4 text-[9px] leading-5 text-[#7EAE9F]">
          <strong className="text-[#8FE7C8]">Produção:</strong> quando <InlineCode>routing.mode</InlineCode> é <InlineCode>LIVE</InlineCode> e o status é <InlineCode>PENDING_PAYMENT</InlineCode>, use <InlineCode>action.copyPaste</InlineCode> para apresentar o PIX ao cliente. A confirmação final deve vir pelo webhook assinado ou pela consulta de status.
        </div>
      </DocsSection>
